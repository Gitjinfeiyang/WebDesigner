    ...
    <div class="table-wrapper">
      <table>
        ...
        <transition-group tag="tbody" name="list-complete">
          <tr class="item list-complete-item" v-for="(swipe,index) in allSwiper" v-if="allSwiper.length>0" :key="swipe.id">
            <td><label><input v-if="radio!='1'" type="checkbox" v-model="swipe.checked"/><input v-if="radio=='1'" name="checked" :value="swipe.id" type="radio" v-model="radioSwiperSelected" /></label></td>
            ...
            <td>
              ...
              <span class="link iconfont" title="下移" @click="exchangeSwiper(swipe.id,allSwiper[index+1].id)">&#xe701;</span>
            </td>
          </tr>
          <tr v-if="allSwiper.length<=0" key="loading">
            <td colspan="4" class="loading"><i class="iconfont">&#xe703;</i></td>
          </tr>
          <tr key="save"><td colspan="4"><loading-button :clickHandler="setSwiper" :loading="settingSwiper">保存</loading-button></td></tr>
        </transition-group>
      </table>
    </div>
    <div class="block">
      <div class="title-bar">新增图片</div>
      <div class="content">
        <div class="input-line">
          <div class="input-title">图片</div>
          <div class="input-content"><img-uploader :preImg="newPhoto.url" @success="getImg"></img-uploader></div>
        </div>
        <div class="input-line">
      ...
      <div class="button-wrapper"><loading-button :clickHandler="upgradeSwiper">保存</loading-button></div>
    </div>
  </div>
</template>
    ...
    export default {
      ...
      methods:{
        ...
        upgradeSwiper(){
            ...
            API.upgradeSwiper({userId:51,...this.newPhoto})
              .then((res) => {
...
<style scoped lang="less">
  ...
  .swiper-setting{
    .block{
      .input-title{flex:0 90px;}
    }
  }
  .table-wrapper{margin-bottom: 40px;}
.item{
  label{display: inline-block; width:100%; height:100%;}
  input[type=checkbox]{width:14px; height:14px; opacity:0.5;}
  td{
    height:48px; padding:12px 10px;  line-height: 48px;
  }
